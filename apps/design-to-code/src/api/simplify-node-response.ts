...
function parseNode(
  ...
  if (hasValue('style', n) && Object.keys(n.style).length) {
    const style = n.style as Record<string, any>;
    const textStyle: TextStyle = {
      fontFamily: style.fontFamily,
      fontWeight: style.fontWeight,
      fontSize: style.fontSize,
      lineHeight:
        style.lineHeightPx !== undefined && style.fontSize
  ...
  if (hasValue('children', n) && Array.isArray(n.children) && n.children.length > 0) {
    // 创建一个副本以避免类型错误
    const childrenArray = [...n.children];

    // 使用类型断言处理 isVisible 和 filter
    const visibleChildren = childrenArray.filter((child) => {
      try {
        return isVisible(child as any);
      } catch (error) {
        console.warn('Error filtering child node:', error);
        return false;
      }
    });

    // 处理子节点
    const processedChildren = visibleChildren
      .map((child) => parseNode(globalVars, child as FigmaDocumentNode, n))
      .filter((child): child is SimplifiedNode => child !== null && child !== undefined);

    if (processedChildren.length) {
      simplified.children = processedChildren;
    }
  }
